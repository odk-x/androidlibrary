...
final class UTMConverter {
  ...
  private static double footpointLatitude(double y) {
    ...
    double alpha_ = (SM_A + SM_B) / 2.0 * (1 + Math.pow(n, 2.0) / 4 + Math.pow(n, 4.0) / 64);
    ...
    return y_ + beta_ * Math.sin(2.0 * y_) + gamma_ * Math.sin(4.0 * y_) + delta_ * Math
        .sin(6.0 * y_) + epsilon_ * Math.sin(8.0 * y_);
  }
  ...
  private static void mapXYToLatLon(double x, double y, double lambda0, double[] philambda) {
    ...
    double x4poly =
        5.0 + 3.0 * tf2 + 6.0 * nuf2 - 6.0 * tf2 * nuf2 - 3.0 * (nuf2 * nuf2) - 9.0 * tf2 * (nuf2
            * nuf2);
    ...
    philambda[0] = phif + x2frac * x2poly * (x * x) + x4frac * x4poly * Math.pow(x, 4.0)
        ...
        /* Calculate longitude */
    philambda[1] =
        lambda0 + x1frac * x + x3frac * x3poly * Math.pow(x, 3.0) + x5frac * x5poly * Math
            .pow(x, 5.0) + x7frac * x7poly * Math.pow(x, 7.0);
